  ...
  <div class="space-y-2">
    ...
    >
      <div class="min-w-0 flex-1 pr-4">
        <span class="font-mono font-semibold text-zinc-50 hover:text-indigo-400">{{ pkg.name }}</span>
        <p v-if="pkg.summary" class="mt-1 text-sm text-zinc-400 line-clamp-2">{{ pkg.summary }}</p>
      </div>
      <span class="shrink-0 font-mono text-xs text-zinc-500">{{ formatDownloads(pkg.downloads) }}/mo</span>
    </NuxtLink>
  </div>
</template>
